...
import { SEARCH_FILTER_PAGE } from "settings/constant";
import {
  FormWrapper,
  ComponentWrapper,
  RoomGuestWrapper,
...
const SearchForm = ({ history, data }) => {
  ...
  const updatevalueFunc = (event) => {
    ...
    setsearchAddress(
      `address[]=${searchedPlaceAPIData[0].formatted_address.replace(
        /\s/g,
        ""
      )}`
    );
    console.log(
      searchedPlaceAPIData[0].formatted_address.replace(/\s/g, ""),
      "tets",
  ...
  return (
    <FormWrapper>
      ...
      <Button
        style={{
          background: "linear-gradient(to right, #40915f, #00FFFF)",
        }}
        type="primary"
        htmlType="submit"
        size="large"
